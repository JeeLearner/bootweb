package com.jee.boot.gen.utils;
...
import com.jee.boot.gen.config.GenConfig;
...
public class GenUtils {
    ...
    public static List<ColumnInfo> transColums(List<ColumnInfo> columns){
        ...
        for (ColumnInfo column : columns) {
            // 列名转换成Java属性名
            String attrName = JeeStringUtils.convertToCamelCase(column.getColumnName());
            column.setAttrName(attrName);
            column.setAttrname(JeeStringUtils.unCapitalize(attrName));
    ...
    public static VelocityContext getVelocityContext(TableInfo table) {
        ...
        velocityContext.put("classname", table.getClassname());
        velocityContext.put("classNameWithPre", table.getClassNameWithPre());
        ...
        velocityContext.put("datetime", DateUtils.now());
    ...
    public static String tableToJava(String tableName) {
        ...
        if (Constants.AUTO_REOMVE_PRE.equals(autoRemovePre) && JeeStringUtils.isNotEmpty(tablePrefix)) {
            tableName = tableName.replaceFirst(tablePrefix, "");
        }
        return JeeStringUtils.convertToCamelCase(tableName);
    }
    ...
    public static String getFileName(String template, TableInfo table, String moduleName) {
        ...
        // 大写类名
        String className = table.getClassName();
        String classNameWithPre = table.getClassNameWithPre();
        String javaPath = PROJECT_PATH;
        String mybatisPath = MYBATIS_PATH + "/" + moduleName + "/" + classNameWithPre;
        String htmlPath = TEMPLATES_PATH + "/" + moduleName + "/" + classname;

        if (template.contains("domain.java.vm")) {
            return javaPath + "domain" + "/" + classNameWithPre + ".java";
        }

        if (template.contains("DTO.java.vm")) {
            return javaPath + "dto" + "/" + classNameWithPre + "DTO.java";
        }

        if (template.contains("VO.java.vm")) {
            return javaPath + "vo" + "/" + classNameWithPre + "VO.java";
        }

        if (template.contains("Mapper.java.vm")) {
            return javaPath + "mapper" + "/" + classNameWithPre + "Mapper.java";
        }

        if (template.contains("Service.java.vm")) {
            return javaPath + "service" + "/" + "I" + classNameWithPre + "Service.java";
        }

        if (template.contains("ServiceImpl.java.vm")) {
            return javaPath + "service" + "/impl/" + classNameWithPre + "ServiceImpl.java";
        }

        if (template.contains("Controller.java.vm")) {
            return javaPath + "controller" + "/" + moduleName + "/" + classNameWithPre + "Controller.java";
        }
    ...
    public static String getModuleName(String packageName) {
        ...
        String moduleName = JeeStringUtils.substring(packageName, lastIndex + 1, nameLength);
        return moduleName;
    }

    public static String getBasePackage(String packageName) {
        int lastIndex = packageName.lastIndexOf(".");
        String basePackage = JeeStringUtils.substring(packageName, 0, lastIndex);
        return basePackage;
    }
