 ...
 */
/**============================================================================
    注意：option定义、chart、数据处理都放到一起，否则在不同的script，无法共享变量
=============================================================================*/
//地图
var dom_map = document.getElementById("map");
var mapChart = echarts.init(dom_map);
//一周访客数
var dom_visit = document.getElementById("visit");
var visitChart = echarts.init(dom_visit);
...
$(document).ready(function(){
    $.ajax({
        type: "post",
        url: ctx + "main/weekUser",
        data: {
        },
        success: function (r) {
            if (r.code == 0){
                for (var i=0; i<r.data.list.length; i++){
                    date.push(r.data.list[i].loginDate);
                    count.push(r.data.list[i].count);
                    outer.push(r.data.list[i].outer);
                }
            } else {
                $.modal.msg(r.msg);
            }
            visitOption.xAxis.data = date;
            visitOption.series[0].data = count;
            visitOption.series[1].data = outer;
            ...
            timeTicket = setInterval(function () {
                // 获取到图表的  option
                visitOption = visitChart.getOption();
                var countArr = visitOption.series[0].data;
                countArr.push(Math.round(Math.random() * 100));
                countArr.shift();
                var outArr = visitOption.series[1].data;
                outArr.push(Math.round(Math.random() * 500));
                outArr.shift();
                visitChart.setOption(visitOption);
